...
```text
  独立仓 SoT（本仓外）                    Agents-Skill-Site（本仓）
  agents-skill-standards                  ┌─── 统一索引 ─────────────────────┐
    standards/**                          │  manifest.json                   │
    角色 · 技能 · 规则                     │  ├─ agents[] / skills[] / rules[] │
         │                                │  ├─ domains[] / health.json      │
         │  npm run sync（可选）           │  Code/code/docs/**（已入库派生页） │
         └───────────────────────────────►│         ▲              ▲         │
                                          └─────────┼──────────────┼─────────┘
                                                    │              │
                      ┌─────────────────────────────┤              │
                ...
                ┌──────────────────┐                │  │  消费端 B                 │
                │  消费端 A        │                │  │  MCP 服务                 │
                │  静态网页        │                │  │  catalog-api.mjs          │
                │  VitePress SSG   │                │  │  stdio / HTTP             │
                │  → dist/         │                │  └───────────┬──────────────┘
                └──────────────────┘                │              │
                                                    └──────┬───────┘
                                                           ▼
...
┌─────────────────────────────────────────────────────────┐
│  治理资产 SoT（独立仓，非本仓子目录）                        │
│  agents / skills / rules 正文 = 磁盘 Markdown             │
└───────────────────────────┬─────────────────────────────┘
                            │ sync（只读派生）
┌───────────────────────────▼─────────────────────────────┐
│  发现层（本仓）                                            │
