...
本项目要做的是：**自建一个网站**，把上述资产以「目录树 + 标签 + 说明书」的方式运行起来，降低发现与使用成本。
...
| 原则 | 在本站的含义 |
|------|----------------|
| 磁盘是 source of truth | Markdown / `.mdc` 仍在 `standards/`；站点只读索引与渲染 |
