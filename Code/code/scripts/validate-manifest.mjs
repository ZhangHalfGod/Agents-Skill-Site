...
const HEALTH_PAGE_ZH = path.join(DOCS_ROOT, 'zh', 'health', 'index.md')
...
function writeHealthPage(health) {
  const linesEn = [
    ...
    `| Validated at | \`${health.validatedAt}\` |`,
    `| Content hash | \`${health.contentHash}\` |`,
    `| Agents | ${health.counts.agents} (expect 8) |`,
    `| Skills | ${health.counts.skills} (expect 11) |`,
  ...
  if (health.errors?.length) {
    linesEn.push('## Errors', '')
    for (const e of health.errors) linesEn.push(`- ${e}`)
    linesEn.push('')
  }
  fs.mkdirSync(path.dirname(HEALTH_PAGE), { recursive: true })
  fs.writeFileSync(HEALTH_PAGE, linesEn.join('\n'), 'utf8')
  ...
  if (health.errors?.length) {
    linesZh.push('## 错误', '')
    for (const e of health.errors) linesZh.push(`- ${e}`)
    linesZh.push('')
  }
  fs.mkdirSync(path.dirname(HEALTH_PAGE_ZH), { recursive: true })
  fs.writeFileSync(HEALTH_PAGE_ZH, linesZh.join('\n'), 'utf8')
}
...
function main() {
  ...
  manifest.note = 'SoT=本仓 docs/zh/**；英文 root 为 stub；agents + skills 1～11 + rules；domains 灰度'
