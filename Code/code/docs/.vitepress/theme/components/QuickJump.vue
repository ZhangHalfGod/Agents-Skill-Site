...
const input = ref('')
const hint = ref('示例：使用角色 2　或　使用技能 1')
...
function go() {
  ...
  let m = text.match(/使用角色\s*(\d+)/i) || text.match(/^角色\s*(\d+)$/i) || text.match(/^r(?:ole)?\s*(\d+)$/i)
  if (m) {
  ...
  m = text.match(/使用技能\s*(\d+)/i) || text.match(/^技能\s*(\d+)$/i) || text.match(/^s(?:kill)?\s*(\d+)$/i)
  if (m) {
  ...
  hint.value = '请输入「使用角色 N」或「使用技能 N」'
}
</script>
  ...
  <div class="quick-jump">
    ...
    <div class="row">
      <input
        v-model="input"
        type="text"
        placeholder="使用角色 2 / 使用技能 1"
        @keyup.enter="go"
      />
      <button type="button" @click="go">跳转</button>
    </div>
    <p class="hint">{{ hint }}</p>
  </div>
</template>
